...
- **Mobile layout is verified but not device-tested:** every route was measured at 375x812 and 768x1024 with no horizontal overflow, and the meeting room, dashboard drawer and lobby were exercised at phone width. Camera capture is blocked in the automated browser, so video tiles were checked with the camera-off placeholder rather than a live stream — worth one pass on a real handset before a demo. Details in `docs/CORE_MEETING_GAP_ANALYSIS.md`.
